...
def validate_model(model, fileW, population, TrainX, TrainY, ValidateX, ValidateY, TestX, TestY):
    ...
    for i in range(numOfPop):
        ...
        #print str(len(X_train_masked))+" train_masked"
        ...
        q2_loo = FromDataFileGA.getTwoDecPoint(q2_loo)

        r2pred_validation = r2Pred(TrainY, ValidateY, Yhat_validation)
        r2pred_validation = FromDataFileGA.getTwoDecPoint(r2pred_validation)

        r2pred_test = r2Pred(TrainY, TestY, Yhat_test)
        r2pred_test = FromDataFileGA.getTwoDecPoint(r2pred_test)
        ...
        trackFitness[idx] = FromDataFileGA.getTwoDecPoint(fitness[i])

        trackModel[idx] = model_desc

        trackR2[idx] = FromDataFileGA.getTwoDecPoint(r2_train)
        trackQ2[idx] = FromDataFileGA.getTwoDecPoint(q2_loo)
        trackR2PredValidation[idx] = FromDataFileGA.getTwoDecPoint(r2pred_validation)
        trackR2PredTest[idx] = FromDataFileGA.getTwoDecPoint(r2pred_test)

        yTrain[idx] = TrainY.tolist()

        yHatTrain[idx] = Yhat_train.tolist()
        for i in range(len(yHatTrain[idx])):
            yHatTrain[idx][i] = FromDataFileGA.getTwoDecPoint(yHatTrain[idx][i])

        yHatCV[idx] = Yhat_cv.tolist()
        for i in range(len(yHatCV[idx])):
            yHatCV[idx][i] = FromDataFileGA.getTwoDecPoint(yHatCV[idx][i])

        yValidation[idx] = ValidateY.tolist()

        yHatValidation[idx] = Yhat_validation.tolist()
        for i in range(len(yHatValidation[idx])):
            yHatValidation[idx][i] = FromDataFileGA.getTwoDecPoint(yHatValidation[idx][i])

        yTest[idx] = TestY.tolist()

        yHatTest[idx] = Yhat_test.tolist()
        for i in range(len(yHatTest[idx])):
            yHatTest[idx][i] = FromDataFileGA.getTwoDecPoint(yHatTest[idx][i])

        """output of the model to the file"""
...
def write(model, fileW, trackDesc, trackFitness, trackModel, trackR2, \
    ...
    for key in trackFitness.keys():
        """ """
        #print str(trackFitness[key])+" "+str(trackR2[key])
